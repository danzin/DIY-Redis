...
export function detectServerRole(args: string[]) {
  ...
  if (replicaOfIndex !== -1 && args[replicaOfIndex + 1]) {
    ...
    if (masterArgs.length > 0) {
      const [host, portStr] = masterArgs[0].includes(' ') 
        ? masterArgs[0].split(' ') 
        : [masterArgs[0], masterArgs[1]];
  ...
  console.log(`Final configuration: port=${port}, role=${role}, masterHost=${masterHost}, masterPort=${masterPort}, dir=${dir}, dbfilename=${dbfilename}`);
