...
export class MasterConnectionHandler extends EventEmitter {
  ...
  private processHandshakeMessage(): number {
    ...
    if (response === '+PONG' && this.handshakeStep === 0) { this.handshakeStep = 1; sendReplconfPort(this.masterConnection, this.replicaPort); }
    else if (response === '+OK' && this.handshakeStep === 1) { this.handshakeStep = 2; sendReplconfCapa(this.masterConnection); }
    else if (response === '+OK' && this.handshakeStep === 2) { this.handshakeStep = 3; sendPsync(this.masterConnection); }
  ...
  private processPropagatedCommand(): number {
    ...
    for (let i = 0; i < arrayLength; i++) {
      ...
      const bulkStringLength = parseInt(this.receivedDataBuffer.slice(currentIndex + 1, endOfBulkStringHeader).toString(), 10);
    ...
    console.log(`Processed command. Total bytes processed: ${currentIndex}. New processing offset: ${this.processingOffset}`);
    // Return the total length of the command
    return currentIndex;
  }

  
  private onError(err: Error) { console.error(`Error connecting to master: ${err.message}`); }
  private onEnd() { console.log('Disconnected from master.'); }
